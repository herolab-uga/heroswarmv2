...
class Controller:
    ...
    def __init__(self):
        ...
        if self.environment_sensor:
            ...
            self.environment_timer = rospy.Timer(rospy.Duration(1/30),self.read_environment,args=(self.sensor_data,))

        # Creates a publisher for imu data
        if self.imu_sensor:
            self.imu_pub = rospy.Publisher("imu", Imu, queue_size=1)
            self.imu_timer = rospy.Timer(rospy.Duration(1/60),self.read_imu,args=(self.sensor_data,))

        # Creates a publisher for the light sensor
        if self.light_sensor:
            self.light_pub = rospy.Publisher('light', Light, queue_size=1)
            self.light_timer = rospy.Timer(rospy.Duration(1/10),self.read_light,args=(self.sensor_data,))
        ...
        if self.proximity_sensor:
            ...
            self.environment_timer = rospy.Timer(rospy.Duration(1/25),self.read_proximity,args=(self.sensor_data,))
