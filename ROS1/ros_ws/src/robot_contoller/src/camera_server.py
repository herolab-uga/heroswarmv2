...
class CameraServer():
    ...
    def __init__(self):
        ...
        orient = 0.0
        apriltag.add_argument(self.parser)
        ...
        self.pos_pub = rospy.Publisher("Positions".Robot_Pos,self.get_positions,10)
    ...
    def get_positions(self):
        ...
        if success:
            ...
            try:
                ...
                ref_x = detections[self.reference_tags[1]].center
                ref_y = detections[self.reference_tags[2]].center
                orig = detections[self.reference_tags[0]].center
            except IndexError:
                return
            ...
            for (i, detection) in enumerate(detections):
                ...
                if not detection.tag_id in self.reference_tags:
                    ...
                    if not detection.tag_id in self.reference_tags:
                        ...
                        positions.append(Odometry())
                        positions[-1].pose.pose.position.x = center_transform[0]
                        positions[-1].pose.pose.position.y = 0 
                        positions[-1].pose.pose.position.z = center_transform[1]

                        q = self.quaternion_from_rpy(np.radians(angle))

                        positions[-1].pose.pose.orientation.x = q[0]
                        positions[-1].pose.pose.orientation.y = q[1]
                        positions[-1].pose.pose.orientation.z = q[2]
                        positions[-1].pose.pose.orientation.w = q[3]
            ...
            if len(detections) == 0 and positions == None:
                overlay = frame
    ...
    def headingDir(self, corners, center):
